...
public class iTweenEvent : MonoBehaviour{
    ...
    public static Dictionary<string, iTweenEvent> paths1 = new Dictionary<string, iTweenEvent>();
    public bool initialized = false;
    public string initialName = "";

    void OnEnable()
    {
        paths1.Add(pathName.ToLower(), this);
    }
    ...
    public static Vector3[] GetPath(string requestedName)
    {
        ...
        if (paths1.ContainsKey(requestedName))
        {
            return paths1[requestedName].nodes.ToArray();
        }
        else
        {
	...
	IEnumerator StartEvent() {
		...
		foreach(var pair in Values) {
			if("path" == pair.Key && pair.Value.GetType() == typeof(string)) optionsHash.Add(pair.Key, iTweenEvent.GetPath((string)pair.Value));
